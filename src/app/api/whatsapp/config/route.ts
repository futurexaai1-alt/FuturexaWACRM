...
export async function POST(request: Request) {
  try {
    ...
    // Futurexa.ai is single-tenant-per-WhatsApp-number — letting two accounts
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one Futurexa.ai user.',
        },
        { status: 409 }
      )
    }
