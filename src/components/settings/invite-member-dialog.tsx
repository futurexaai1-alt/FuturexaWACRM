...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Futurexa.ai account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Futurexa.ai account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our Futurexa.ai account';
    const message = `Join ${accountName} on Futurexa.ai using this link (valid for ${result?.expiresInDays} days): ${url}`;
          ...
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The Futurexa.ai Button is the Base UI
